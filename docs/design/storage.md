...
| **Concurrent Writes (WAL Mode)** | $\gt 7\text{--}17\text{ writes/sec}$ on a single stream | Direct client conditional create congestion on sequence numbers. One conditional create costs one round trip, and the contiguity invariant serializes them. A measurement from a us-central1 VM against a us-central1 bucket peaked at 16.9 appends/sec, and a laptop reached 7.5. | `WALWriter Service` gateway micro-batching into RecordIO streams |
