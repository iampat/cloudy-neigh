...
func TestTenantCatalogJSONRoundTrip(t *testing.T) {
	catalog := &namespacepb.TenantCatalog{
		...
		Namespaces: map[string]*namespacepb.NamespaceMetadata{
			"analytics": {
				Status:    namespacepb.NamespaceStatus_DELETED,
				Epoch:     1,
...
func TestReadTenantCatalogIfGeneration(t *testing.T) {
	...
	assert.Equal(t, namespacepb.NamespaceStatus_ACTIVE, catalog.Namespaces["catalog"].Status)
...
func TestCreateNamespaceRetriesCASConflict(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, namespacepb.NamespaceStatus_ACTIVE, meta.Status)
	...
	assert.Equal(t, namespacepb.NamespaceStatus_ACTIVE, catalog.Namespaces["catalog"].Status)
	assert.Equal(t, namespacepb.NamespaceStatus_ACTIVE, catalog.Namespaces["other"].Status)
}
...
func TestDeleteNamespaceCannotBeRecreated(t *testing.T) {
	...
	assert.Equal(t, namespacepb.NamespaceStatus_ACTIVE, created.Status)

	deleted, _, err := namespace.DeleteNamespace(ctx, store, "acme", "analytics", time.Unix(1773500000, 0))
	require.NoError(t, err)
	assert.Equal(t, namespacepb.NamespaceStatus_DELETED, deleted.Status)
	...
	require.NoError(t, err)
	assert.Equal(t, namespacepb.NamespaceStatus_DELETED, deletedAgain.Status)
...
func (s *injectConflictStore) Put(ctx context.Context, key string, r io.Reader, cond objectstore.Condition) (string, error) {
	...
	if !s.injected && key == pathForTenant(s.tenant) && cond.Absent {
		s.injected = true
		_, err := s.Store.Put(ctx, key, strings.NewReader(`{"tenant":"acme","version":1,"namespaces":{"other":{"status":"ACTIVE","epoch":0,"created_at":1773990000}}}`), objectstore.Condition{Absent: true})
		if err != nil {
			return "", err
		}
