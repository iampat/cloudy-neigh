...
* **Mandatory Vanilla Baseline Across All Stages:** In all stages (brainstorm, design, implement, debug), always start with a vanilla solution. Never introduce fancy structures, bit-packing, or premature complexity upfront. Premature optimization is the root of all evil 97% of the time. Only optimize when profiling proves the vanilla implementation is a bottleneck.
* **Read Optimization Over Write Optimization:** Always favor read performance over write performance. Design data layouts and query paths to minimize query latency, even if writes require full copies.
