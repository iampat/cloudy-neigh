...
Corrections Ali repeated that no rule file covers. Read this before the first
action in a task.
...
rest, and the repeat offence that a rule states already but I break anyway.

The `retro` skill maintains this file, and Ali approves every change it makes.
Do not edit this file by hand, and do not add an entry from one case.
