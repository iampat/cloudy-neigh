...
- [x] Delete `singleflight.Group` (`flushGroup`, `readGroup`) from `kvfs/store.go`.
- [x] Pass caller `context.Context` through all I/O in `Flush` and `loadManifest`.
- [x] Remove lease TTL cache and fix `Manifest` pointer aliasing race on reads.
- [x] Delete unused `existsBlob` and `existsCAS` in `kvfs/cas.go`.
